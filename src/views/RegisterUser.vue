...
export default {
    ...
    methods: {
        async cargarUsuarios() {
            try {
                const response = await fetch('http://192.168.1.5:34567/api/usuarios');
        ...
        async guardarUsuario() {
            try {
                const url = this.esEdicion
                    ? `http://192.168.1.5:34567/api/usuarios/${this.nuevoUsuario.id}`
                    : 'http://192.168.1.5:34567/api/usuarios';
        ...
        async eliminarUsuario(id) {
            ...
            if (result.isConfirmed) {
                try {
                    const response = await fetch(`http://192.168.1.5:34567/api/usuarios/${id}`, {
                        method: 'DELETE'
                    });
